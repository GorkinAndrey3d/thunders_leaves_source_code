...
//-----------------------------------------------------------------------------
class CHud_EHHelper : public CHudElement, public CHudNumericDisplay
{
	DECLARE_CLASS_SIMPLE(CHud_EHHelper, CHudNumericDisplay);

public:
	CHud_EHHelper(const char *pElementName);
...
private:
	...
	CPanelAnimationVarAliasType(float, text2_xpos, "text_xpos", "4", "proportional_float");
	CPanelAnimationVarAliasType(float, text2_ypos, "text_ypos", "85", "proportional_float");
...
DECLARE_HUDELEMENT(CHud_EHHelper);
DECLARE_HUD_MESSAGE(CHud_EHHelper, Damage);

//-----------------------------------------------------------------------------
// Purpose: Constructor
//-----------------------------------------------------------------------------
CHud_EHHelper::CHud_EHHelper(const char *pElementName) : CHudElement(pElementName), CHudNumericDisplay(NULL, "CHud_EHHelper")
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CHud_EHHelper::Init()
{
	HOOK_HUD_MESSAGE(CHud_EHHelper, Damage);
	Reset();
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CHud_EHHelper::MsgFunc_Damage(bf_read &msg)
{
...
static ConVar show_electrohackmode("show_electrohackmode", "0", 0, "toggles beta icon in upper right corner");

void CHud_EHHelper::togglePrint()
{

	if (!show_electrohackmode.GetBool())
