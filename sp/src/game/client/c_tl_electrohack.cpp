...
class C_NPC_Suppressorelectrohack : public C_AI_BaseNPC
{
public:
	C_NPC_Suppressorelectrohack() {}

	DECLARE_CLASS(C_NPC_Suppressorelectrohack, C_AI_BaseNPC);
	DECLARE_CLIENTCLASS();
	DECLARE_DATADESC();
...
private:
	C_NPC_Suppressorelectrohack(const C_NPC_Suppressorelectrohack &);
...
// Save/restore
//-----------------------------------------------------------------------------
BEGIN_DATADESC(C_NPC_Suppressorelectrohack)
...
// Networking
//-----------------------------------------------------------------------------
IMPLEMENT_CLIENTCLASS_DT(C_NPC_Suppressorelectrohack, DT_electrohack, CNPC_Suppressorelectrohack)
...
// Purpose: Start the manhack's engine sound.
//-----------------------------------------------------------------------------
void C_NPC_Suppressorelectrohack::OnDataChanged(DataUpdateType_t type)
{
...
// Restore
//-----------------------------------------------------------------------------
void C_NPC_Suppressorelectrohack::OnRestore()
{
	BaseClass::OnRestore();
	SoundInit();
}
...
// Purpose: Start the manhack's engine sound.
//-----------------------------------------------------------------------------
void C_NPC_Suppressorelectrohack::UpdateOnRemove(void)
{
...
// Purpose: Start the manhack's engine sound.
//-----------------------------------------------------------------------------
void C_NPC_Suppressorelectrohack::SoundInit(void)
{
	if ((m_nEnginePitch1 < 0) || (m_nEnginePitch2 < 0))
		return;
...
// Purpose:
//-----------------------------------------------------------------------------
void C_NPC_Suppressorelectrohack::SoundShutdown(void)
{
	// Kill the engine!
	if (m_pEngineSound1)
	{
