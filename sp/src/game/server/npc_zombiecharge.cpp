...
#define ZOMBIECHARGE_MAX_GRENADES 1

int ACT_ZOMBIECHARGE_GRENADE_PULL;
int ACT_ZOMBIECHARGE_GRENADE_WALK;
int ACT_ZOMBIECHARGE_GRENADE_RUN;
int ACT_ZOMBIECHARGE_GRENADE_IDLE;
int ACT_ZOMBIECHARGE_ATTACK_FAST;
int ACT_ZOMBIECHARGE_GRENADE_FLINCH_BACK;
int ACT_ZOMBIECHARGE_GRENADE_FLINCH_FRONT;
int ACT_ZOMBIECHARGE_GRENADE_FLINCH_WEST;
int ACT_ZOMBIECHARGE_GRENADE_FLINCH_EAST;

int AE_ZOMBIECHARGE_PULLPIN;

extern bool IsAlyxInDarknessMode();

ConVar	sk_zombiecharge_health( "sk_zombiecharge_health","0");

float g_flZombineGrenadeTimes = 0;

class CNPC_ZombieCharge : public CAI_BlendingHost<CNPC_BaseZombie>, public CDefaultPlayerPickupVPhysics
{
	DECLARE_DATADESC();
	DECLARE_CLASS( CNPC_ZombieCharge, CAI_BlendingHost<CNPC_BaseZombie> );
...
public:
	...
	enum
	{
		COND_ZOMBIECHARGE_GRENADE = LAST_BASE_ZOMBIE_CONDITION,
	};

	enum
	{
		SCHED_ZOMBIECHARGE_PULL_GRENADE = LAST_BASE_ZOMBIE_SCHEDULE,
	};
...
LINK_ENTITY_TO_CLASS( npc_zombiecharge, CNPC_ZombieCharge );

BEGIN_DATADESC( CNPC_ZombieCharge )
...
//---------------------------------------------------------
//---------------------------------------------------------
const char *CNPC_ZombieCharge::pMoanSounds[] =
{
	"ATV_engine_null",
};

void CNPC_ZombieCharge::Spawn( void )
{
	Precache();
	...
	m_iHealth			= sk_zombiecharge_health.GetFloat();
	...
	m_iGrenadeCount = ZOMBIECHARGE_MAX_GRENADES;
}

void CNPC_ZombieCharge::Precache( void )
{
	BaseClass::Precache();
...
void CNPC_ZombieCharge::SetZombieModel( void )
{
...
void CNPC_ZombieCharge::PrescheduleThink( void )
{
...
void CNPC_ZombieCharge::OnScheduleChange( void )
{
...
bool CNPC_ZombieCharge::CanRunAScriptedNPCInteraction( bool bForced )
{
	if ( HasGrenade() == true )
		return false;
...
int CNPC_ZombieCharge::SelectSchedule( void )
{
	if ( GetHealth() <= 0 )
		return BaseClass::SelectSchedule();

	if ( HasCondition( COND_ZOMBIECHARGE_GRENADE ) )
	{
		ClearCondition(COND_ZOMBIECHARGE_GRENADE);
		
		return SCHED_ZOMBIECHARGE_PULL_GRENADE;
	}

	return BaseClass::SelectSchedule();
}

void CNPC_ZombieCharge::BuildScheduleTestBits( void )
{
	BaseClass::BuildScheduleTestBits();

	SetCustomInterruptCondition(COND_ZOMBIECHARGE_GRENADE);
}

Activity CNPC_ZombieCharge::NPC_TranslateActivity( Activity baseAct )
{
	if ( baseAct == ACT_MELEE_ATTACK1 )
	{
		if ( m_flSuperFastAttackTime > gpGlobals->curtime || HasGrenade() )
		{
			return (Activity)ACT_ZOMBIECHARGE_ATTACK_FAST;
		}
	}
	...
	if ( baseAct == ACT_IDLE )
	{
		if ( HasGrenade() )
		{
			return (Activity)ACT_ZOMBIECHARGE_GRENADE_IDLE;
		}
	}
...
int CNPC_ZombieCharge::MeleeAttack1Conditions ( float flDot, float flDist )
{
...
void CNPC_ZombieCharge::GatherGrenadeConditions( void )
{
	...
	if ( pPlayer && pPlayer->FVisible( this ) )
	{
		...
		if ( flLengthToPlayer <= GRENADE_PULL_MAX_DISTANCE && flLengthToEnemy <= GRENADE_PULL_MAX_DISTANCE )
		{
			...
			if ( flPullChance >= random->RandomFloat( 0.0f, 1.0f ) )
			{
				...
				SetCondition(COND_ZOMBIECHARGE_GRENADE);
			}
		}
	}
}

int CNPC_ZombieCharge::TranslateSchedule( int scheduleType ) 
{
	return BaseClass::TranslateSchedule( scheduleType );
}

void CNPC_ZombieCharge::DropGrenade( Vector vDir )
{
	if ( m_hGrenade == NULL )
		 return;
...
void CNPC_ZombieCharge::Event_Killed( const CTakeDamageInfo &info )
{
...
//-----------------------------------------------------------------------------
bool CNPC_ZombieCharge::HandleInteraction( int interactionType, void *data, CBaseCombatCharacter *sourceEnt )
{
...
void CNPC_ZombieCharge::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator )
{
...
void CNPC_ZombieCharge::HandleAnimEvent( animevent_t *pEvent )
{
	if (pEvent->event == AE_ZOMBIECHARGE_PULLPIN)
	{
		Vector vecStart;
		QAngle angles;
...
bool CNPC_ZombieCharge::AllowedToSprint( void )
{
	if ( IsOnFire() )
		return false;
...
void CNPC_ZombieCharge::StopSprint( void )
{
...
void CNPC_ZombieCharge::Sprint( bool bMadSprint )
{
	if ( IsSprinting() )
		return;
...
void CNPC_ZombieCharge::RunTask( const Task_t *pTask )
{
		...
		case TASK_WAIT_FOR_MOVEMENT:
		{
			...
			if ( GetEnemy() )
			{
				...
				if ( HasGrenade() )
				{
					if ( IsSprinting() )
					{
						GetNavigator()->SetMovementActivity((Activity)ACT_ZOMBIECHARGE_GRENADE_RUN);
					}
					else
					{
						GetNavigator()->SetMovementActivity((Activity)ACT_ZOMBIECHARGE_GRENADE_WALK);
					}
...
void CNPC_ZombieCharge::InputStartSprint ( inputdata_t &inputdata )
{
	Sprint();
}

void CNPC_ZombieCharge::InputPullGrenade ( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
const char *CNPC_ZombieCharge::GetMoanSound( int nSound )
{
...
//-----------------------------------------------------------------------------
void CNPC_ZombieCharge::FootstepSound( bool fRightFoot )
{
	if( fRightFoot )
	{
...
//-----------------------------------------------------------------------------
bool CNPC_ZombieCharge::ShouldBecomeTorso( const CTakeDamageInfo &info, float flDamageThreshold )
{
	return false;
}
...
//-----------------------------------------------------------------------------
void CNPC_ZombieCharge::FootscuffSound( bool fRightFoot )
{
	if( fRightFoot )
	{
...
//-----------------------------------------------------------------------------
void CNPC_ZombieCharge::AttackHitSound( void )
{
...
//-----------------------------------------------------------------------------
void CNPC_ZombieCharge::AttackMissSound( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_ZombieCharge::PainSound( const CTakeDamageInfo &info )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_ZombieCharge::DeathSound( const CTakeDamageInfo &info ) 
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_ZombieCharge::AlertSound( void )
{
...
//-----------------------------------------------------------------------------
void CNPC_ZombieCharge::IdleSound( void )
{
...
//-----------------------------------------------------------------------------
void CNPC_ZombieCharge::AttackSound( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
const char *CNPC_ZombieCharge::GetHeadcrabModel( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
const char *CNPC_ZombieCharge::GetLegsModel( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
const char *CNPC_ZombieCharge::GetTorsoModel( void )
{
...
//---------------------------------------------------------
void CNPC_ZombieCharge::MoanSound( envelopePoint_t *pEnvelope, int iEnvelopeSize )
{
	if( IsOnFire() )
	{
...
//-----------------------------------------------------------------------------
const char *CNPC_ZombieCharge::GetHeadcrabClassname( void )
{
	return "npc_headcrab";
}

void CNPC_ZombieCharge::ReleaseGrenade( Vector vPhysgunPos )
{
	if ( HasGrenade() == false )
		return;
	...
	if ( flDotRight > flDotForward )
	{
		if ( bNegativeRight == true )
			aActivity = (Activity)ACT_ZOMBIECHARGE_GRENADE_FLINCH_WEST;
		else 
			aActivity = (Activity)ACT_ZOMBIECHARGE_GRENADE_FLINCH_EAST;
	}
	else
	{
		if ( bNegativeForward == true )
			aActivity = (Activity)ACT_ZOMBIECHARGE_GRENADE_FLINCH_BACK;
		else 
			aActivity = (Activity)ACT_ZOMBIECHARGE_GRENADE_FLINCH_FRONT;
	}
...
CBaseEntity *CNPC_ZombieCharge::OnFailedPhysGunPickup( Vector vPhysgunPos )
{
...
AI_BEGIN_CUSTOM_NPC( npc_zombine, CNPC_ZombieCharge )

	//Squad slots
	DECLARE_SQUADSLOT( SQUAD_SLOT_ZOMBINE_SPRINT1 )
	DECLARE_SQUADSLOT( SQUAD_SLOT_ZOMBINE_SPRINT2 )

	DECLARE_CONDITION(COND_ZOMBIECHARGE_GRENADE)

	DECLARE_ACTIVITY(ACT_ZOMBIECHARGE_GRENADE_PULL)
	DECLARE_ACTIVITY(ACT_ZOMBIECHARGE_GRENADE_WALK)
	DECLARE_ACTIVITY(ACT_ZOMBIECHARGE_GRENADE_RUN)
	DECLARE_ACTIVITY(ACT_ZOMBIECHARGE_GRENADE_IDLE)
	DECLARE_ACTIVITY(ACT_ZOMBIECHARGE_ATTACK_FAST)
	DECLARE_ACTIVITY(ACT_ZOMBIECHARGE_GRENADE_FLINCH_BACK)
	DECLARE_ACTIVITY(ACT_ZOMBIECHARGE_GRENADE_FLINCH_FRONT)
	DECLARE_ACTIVITY(ACT_ZOMBIECHARGE_GRENADE_FLINCH_WEST)
	DECLARE_ACTIVITY(ACT_ZOMBIECHARGE_GRENADE_FLINCH_EAST)

	DECLARE_ANIMEVENT(AE_ZOMBIECHARGE_PULLPIN)


	DEFINE_SCHEDULE
	(
	SCHED_ZOMBIECHARGE_PULL_GRENADE,
