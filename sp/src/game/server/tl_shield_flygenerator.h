#ifndef NPC_FLYGENERATOR_H
#define NPC_FLYGENERATOR_H
#ifdef _WIN32
#pragma once
#endif
...
// Manhack 
//-----------------------------------------------------------------------------
class CNPC_SuppressorFlyGenerator : public CNPCBaseInteractive<CAI_BasePhysicsFlyingBot>, public CDefaultPlayerPickupVPhysics
{
	DECLARE_CLASS(CNPC_SuppressorFlyGenerator, CNPCBaseInteractive<CAI_BasePhysicsFlyingBot>);
	DECLARE_SERVERCLASS();

public:
	CNPC_SuppressorFlyGenerator();
	~CNPC_SuppressorFlyGenerator();
	...
	void            EnableDefenceVoid(void);

	void			SpinBlades(float flInterval);
	//void			DefenceStateOne(float flInterval);
	void			BreakControl(float flInterval);
	...
	void            EnableDefence(inputdata_t &inputdata);
	...
	COutputEvent m_nOnBreaking1;
	COutputEvent m_nOnBreaking2;
	COutputEvent m_nOnBreaking3;
	COutputEvent m_nOnBreaking4;
	COutputEvent m_nOnBreaking5;
...
private:
	bool m_bActive;
	bool breaking_1 = false;
	bool breaking_2 = false;
	bool breaking_3 = false;
	bool breaking_4 = false;
	bool breaking_5 = false;
	COutputEvent LastBreak;
	


	float m_DefenceHealth = 50000;
	float m_DefenceHealthSave = 100;
	float m_flNextChangeTime;
	float m_flNextChangeTime2;
	float m_flDefenceTime1;
	float m_flDefenceTime2;
	float m_DefenceTimeEnd;
	float m_DefenceTime;
	bool m_bShieldActive;
	bool DefenceEnabled = false;
	...
	void CreateGlow(bool DefenceEnabledVoid);
	void ChangeHealth(bool DefenceEnabledVoid);
...
class CTLSuppressorShieldAttach2 : public CBaseAnimating
{
public:
	DECLARE_CLASS(CTLSuppressorShieldAttach2, CBaseAnimating);
	DECLARE_DATADESC();
	void Spawn(void);
...
class CFlyGeneratorGib : public CItem
{
public:
	DECLARE_CLASS(CFlyGeneratorGib, CItem);
...
BEGIN_DATADESC(CFlyGeneratorGib)
DEFINE_FIELD(m_nDenomination, FIELD_INTEGER),

END_DATADESC()

LINK_ENTITY_TO_CLASS(item_FlyGeneratorGib, CFlyGeneratorGib);

LINK_ENTITY_TO_CLASS(npc_suppressor_shield_attach2, CTLSuppressorShieldAttach2);

BEGIN_DATADESC(CTLSuppressorShieldAttach2)

END_DATADESC()

#define ENTITY_MODEL_SUP "models/supressor_sphere.mdl"
#define FLYGENERATORONE "models/flygenerator/flygenerator_full.mdl"
#define FLYGENERATORTWO "models/flygenerator/flygenerator_one.mdl"
#define FLYGENERATORTHREE "models/flygenerator/flygenerator_two.mdl"
#define FLYGENERATORFOUR "models/flygenerator/flygenerator_three.mdl"
#define FLYGENERATORNULL "models/flygenerator/flygenerator_four.mdl"
#define FLYGENERATORNULLNULL "models/flygenerator/flygenerator_null.mdl"
#define FLYGENERATORBREAK "models/flygenerator/flygenerator_break.mdl"