...
#include "tl_shield_flygenerator.h"
...
#define HEALTH_START 500
#define HEALTH_BREAK1 490
#define HEALTH_BREAK2 400
#define HEALTH_BREAK3 300
#define HEALTH_BREAK4 200
...
ConVar	sk_flygenerator_health("sk_flygenerator_health", "0");
ConVar	sk_flygenerator_melee_dmg("sk_flygenerator_melee_dmg", "0");
ConVar	sk_flygenerator_v2("sk_flygenerator_v2", "1");
...
// Private activities.
//-----------------------------------------------------------------------------
Activity ACT_FLYGENERATOR_UNPACK;
...
BEGIN_DATADESC(CNPC_SuppressorFlyGenerator)
...
DEFINE_INPUTFUNC(FIELD_VOID, "EnableDefence", EnableDefence),
DEFINE_THINKFUNC(DefenceThink),

DEFINE_OUTPUT(m_nOnBreaking1, "OnFirstBreaking"),
DEFINE_OUTPUT(m_nOnBreaking2, "OnSecondBreaking"),
DEFINE_OUTPUT(m_nOnBreaking3, "OnThirdBreaking"),
DEFINE_OUTPUT(m_nOnBreaking4, "OnFourBreaking"),
DEFINE_OUTPUT(m_nOnBreaking5, "OnFullBreaking"),
...
LINK_ENTITY_TO_CLASS(npc_suppressorflygenerator, CNPC_SuppressorFlyGenerator);

IMPLEMENT_SERVERCLASS_ST(CNPC_SuppressorFlyGenerator, DT_SuppressorFlyGenerator)//
...
// Output  :
//------------------------------------------------------------------------------
CNPC_SuppressorFlyGenerator::CNPC_SuppressorFlyGenerator()
{
#ifdef _DEBUG
...
//-----------------------------------------------------------------------------
Class_T	CNPC_SuppressorFlyGenerator::Classify(void)
{
...
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::Event_Dying(void)
{
	DestroySmokeTrail();
	SetHullSizeNormal();
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::GatherConditions()
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::PrescheduleThink(void)
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::TraceAttack(const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::DeathSound(const CTakeDamageInfo &info)
{
...
//-----------------------------------------------------------------------------
bool CNPC_SuppressorFlyGenerator::ShouldGib(const CTakeDamageInfo &info)
{
	return (m_bGib);
}
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::Event_Killed(const CTakeDamageInfo &info)
{
	// turn off the blur!
...
void CNPC_SuppressorFlyGenerator::HitPhysicsObject(CBaseEntity *pOther)
{
...
void CNPC_SuppressorFlyGenerator::TakeDamageFromPhyscannon(CBasePlayer *pPlayer)
{
...
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::TakeDamageFromVehicle(int index, gamevcollisionevent_t *pEvent)
{
...
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::TakeDamageFromPhysicsImpact(int index, gamevcollisionevent_t *pEvent)
{
...
void CNPC_SuppressorFlyGenerator::VPhysicsCollision(int index, gamevcollisionevent_t *pEvent)
{
...
void CNPC_SuppressorFlyGenerator::VPhysicsShadowCollision(int index, gamevcollisionevent_t *pEvent)
{
	int otherIndex = !index;
	CBaseEntity *pOther = pEvent->pEntities[otherIndex];
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::CrashTouch(CBaseEntity *pOther)
{
...
// Create smoke trail!
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::CreateSmokeTrail()
{
...
void CNPC_SuppressorFlyGenerator::DestroySmokeTrail()
{
...
// Output :
//-----------------------------------------------------------------------------
int	CNPC_SuppressorFlyGenerator::OnTakeDamage_Alive(const CTakeDamageInfo &info)
{
...
// Purpose:
//------------------------------------------------------------------------------
bool CNPC_SuppressorFlyGenerator::CorpseGib(const CTakeDamageInfo &info)
{
...
// Output :
//-----------------------------------------------------------------------------
int	CNPC_SuppressorFlyGenerator::OnTakeDamage_Dying(const CTakeDamageInfo &info)
{
...
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::OnStateChange(NPC_STATE OldState, NPC_STATE NewState)
{
...
// Input  : Type - 
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::HandleAnimEvent(animevent_t *pEvent)
{
...
//-----------------------------------------------------------------------------
bool CNPC_SuppressorFlyGenerator::IsFlyingActivity(Activity baseAct)
{
...
// Input  : Type - 
//-----------------------------------------------------------------------------
Activity CNPC_SuppressorFlyGenerator::NPC_TranslateActivity(Activity baseAct)
{
...
// Input  : Type - 
//-----------------------------------------------------------------------------
int CNPC_SuppressorFlyGenerator::TranslateSchedule(int scheduleType)
{
...
void CNPC_SuppressorFlyGenerator::Loiter()
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::MaintainGroundHeight(void)
{
...
// Input  : flInterval - 
//-----------------------------------------------------------------------------
bool CNPC_SuppressorFlyGenerator::OverrideMove(float flInterval)
{
	
	SpinBlades(flInterval);
	BreakControl(flInterval);
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::TurnHeadRandomly(float flInterval)
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::MoveToTarget(float flInterval, const Vector &vMoveTarget)
{
...
void CNPC_SuppressorFlyGenerator::EnableDefence(inputdata_t &inputData)
{
	m_DefenceTimeEnd = gpGlobals->curtime + float(10.0f);
	m_DefenceTime = float(0.0f);
	if (m_DefenceTime < m_DefenceTimeEnd)
	{
		Vector SparkOrigin = GetAbsOrigin();
		Vector vecDir33 = vec3_origin;
		DoSpark(this, SparkOrigin, 1.0f, 1.0f, true, vecDir33);
		m_DefenceTime = gpGlobals->curtime + float(1.0f);
...
void CNPC_SuppressorFlyGenerator::EnableDefenceVoid(void)
{
		Vector SparkOrigin = GetAbsOrigin();
		Vector vecDir33 = vec3_origin;
		DoSpark(this, SparkOrigin, 1.0f, 1.0f, true, vecDir33);
...
void CNPC_SuppressorFlyGenerator::DefenceThink(void)
{
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_SuppressorFlyGenerator::MoveCollisionMask(void)
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::Splash(const Vector &vecSplashPos)
{
	CEffectData	data;
...
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::ComputeSliceBounceVelocity(CBaseEntity *pHitEntity, trace_t &tr)
{
...
//-----------------------------------------------------------------------------
bool CNPC_SuppressorFlyGenerator::IsHeldByPhyscannon()
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::Slice(CBaseEntity *pHitEntity, float flInterval, trace_t &tr)
{
	...
	float flDamage = sk_flygenerator_melee_dmg.GetFloat() * flInterval;
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::Bump(CBaseEntity *pHitEntity, float flInterval, trace_t &tr)
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::CheckCollisions(float flInterval)
{
...
void CNPC_SuppressorFlyGenerator::PlayFlySound(void)
{
	float flEnemyDist;
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::MoveExecute_Alive(float flInterval)
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::SpinBlades(float flInterval)
{
	if (!m_bBladesActive)
	{
...
/*
void CNPC_SuppressorFlyGenerator::DefenceStateOne(float flInterval)
{
	...
	if (IsFlyingActivity(GetActivity()))
	{
		...
		if (m_flNextChangeTime < gpGlobals->curtime)
		{
                ...
                CreateGlow(m_DefenceGlowState);
				m_DefenceGlowState = !m_DefenceGlowState;
...
*/
void CNPC_SuppressorFlyGenerator::BreakControl(float flInterval)
{
	if (m_iHealth < HEALTH_BREAK1 && breaking_1 == false)
	{
		SetModel(FLYGENERATORTWO);
		breaking_1 = true;
		CreateNugget();
	}
	if (m_iHealth < HEALTH_BREAK2 && breaking_2 == false)
	{
		SetModel(FLYGENERATORTHREE);
		breaking_2 = true;
		CreateNugget();
		
	}
	if (m_iHealth < HEALTH_BREAK3 && breaking_3 == false)
	{
		SetModel(FLYGENERATORFOUR);
		breaking_3 = true;
		CreateNugget();
	}
	if (m_iHealth < 100 && breaking_4 == false)
	{
		SetModel(FLYGENERATORNULL);
		DoSpark(this, GetAbsOrigin(), 1.0f, 1.0f, true, vec3_origin);
		breaking_4 = true;
		CreateNugget();
		m_nOnBreaking5.FireOutput(this, this);
		Event_Dying();
	}
}

void CNPC_SuppressorFlyGenerator::CreateNugget(void)
{
	CFlyGeneratorGib *pNugget = (CFlyGeneratorGib *)CreateEntityByName("item_FlyGeneratorGib");
	if (pNugget == NULL)
		return;
...
void CNPC_SuppressorFlyGenerator::CreateNugget_last(void)
{
	CFlyGeneratorGib *pNugget = (CFlyGeneratorGib *)CreateEntityByName("item_FlyGeneratorGib");
	if (pNugget == NULL)
		return;
	...
	pNugget->SetAbsAngles(RandomAngle(0, 360));
	DispatchSpawn(pNugget);
	pNugget->SetModel(FLYGENERATORNULLNULL);
	pNugget->SetModelScale(4);
...
void CFlyGeneratorGib::Precache(void)
{
...
bool CFlyGeneratorGib::VPhysicsIsFlesh(void)
{
	return false;
}
...
void CFlyGeneratorGib::VPhysicsCollision(int index, gamevcollisionevent_t *pEvent)
{
	int damageType;
...
void CFlyGeneratorGib::Event_Killed(const CTakeDamageInfo &info)
{
	BaseClass::Event_Killed(info);
}

void CFlyGeneratorGib::Spawn(void)
{
    SetModel(FLYGENERATORBREAK);
...
void CNPC_SuppressorFlyGenerator::CreateGlow(bool DefenceEnabledVoid)
{
	

	
	if (DefenceEnabledVoid == true)
	{
		m_hGlowSprite = CSprite::SpriteCreate(MANHACK_GLOW_SPRITE, GetLocalOrigin(), false);
	...
	if (DefenceEnabledVoid == false)
	{
		m_hGlowSprite->Remove();
	}
}

void CNPC_SuppressorFlyGenerator::ChangeHealth(bool DefenceEnabledVoid)
{
	...
	if (DefenceEnabledVoid == true)
	{
		m_DefenceHealthSave = m_iHealth;
		m_iHealth = m_DefenceHealth;
	}
	if (DefenceEnabledVoid == false)
	{
		m_iHealth = m_DefenceHealthSave;
	}
}
...
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::MoveExecute_Dead(float flInterval)
{
...
void CTLSuppressorShieldAttach2::Precache(void)
{
	PrecacheModel(ENTITY_MODEL_SUP);
}

void CTLSuppressorShieldAttach2::Spawn(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::Precache(void)
{
	//
	// Model.
	//
	PrecacheModel(FLYGENERATORONE);
	PrecacheModel(FLYGENERATORTWO);
	PrecacheModel(FLYGENERATORTHREE);
	PrecacheModel(FLYGENERATORFOUR);
	PrecacheModel(FLYGENERATORNULL);
	PrecacheModel(FLYGENERATORNULLNULL);
	PrecacheModel(FLYGENERATORBREAK);
	PrecacheModel("models/props_combine/combine_barricade_bracket01a.mdl");
	PrecacheModel("models/gibs/airboat_broken_engine.mdl");
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::GatherEnemyConditions(CBaseEntity *pEnemy)
{
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_SuppressorFlyGenerator::MeleeAttack1Conditions(float flDot, float flDist)
{
	if (GetEnemy() == NULL)
		return COND_NONE;
...
// Input  : *pTask - 
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::RunTask(const Task_t *pTask)
{
	switch (pTask->iTask)
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::Spawn(void)
{
	Precache();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::StartEye(void)
{
...
void CNPC_SuppressorFlyGenerator::Activate()
{
	BaseClass::Activate();
...
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::PostNPCInit(void)
{
...
void CNPC_SuppressorFlyGenerator::BladesInit()
{
	if (!m_bBladesActive)
	{
...
// Crank up the engine!
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::StartEngine(bool fStartSound)
{
	if (fStartSound)
	{
		SoundInit();
...
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::SoundInit(void)
{
	m_nEnginePitch1 = MANHACK_MIN_PITCH1;
...
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::StopLoopingSounds(void)
{
...
// Input  : pTask - 
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::StartTask(const Task_t *pTask)
{
	switch (pTask->iTask)
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::UpdateOnRemove(void)
{
	DestroySmokeTrail();
	KillSprites(0.0);
...
//-----------------------------------------------------------------------------
bool CNPC_SuppressorFlyGenerator::HandleInteraction(int interactionType, void* data, CBaseCombatCharacter* sourceEnt)
{
...
// Output : float
//-----------------------------------------------------------------------------
float CNPC_SuppressorFlyGenerator::ManhackMaxSpeed(void)
{
	if (m_flWaterSuspendTime > gpGlobals->curtime)
	{
		// Slower in water!
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::ClampMotorForces(Vector &linear, AngularImpulse &angular)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::KillSprites(float flDelay)
{
	if (m_pEyeGlow)
	{
...
//-----------------------------------------------------------------------------
bool CNPC_SuppressorFlyGenerator::IsInEffectiveTargetZone(CBaseEntity *pTarget)
{
...
//			&tolerance - 
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::TranslateNavGoal(CBaseEntity *pEnemy, Vector &chasePosition)
{
...
float CNPC_SuppressorFlyGenerator::GetDefaultNavGoalTolerance()
{
	return GetHullWidth();
}
...
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::InputDisableSwarm(inputdata_t &inputdata)
{
...
// Input  : &inputdata - 
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::InputUnpack(inputdata_t &inputdata)
{
...
//			reason - 
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::OnPhysGunPickup(CBasePlayer *pPhysGunUser, PhysGunPickup_t reason)
{
...
//			Reason - 
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::OnPhysGunDrop(CBasePlayer *pPhysGunUser, PhysGunDrop_t Reason)
{
...
void CNPC_SuppressorFlyGenerator::StartLoitering(const Vector &vecLoiterPosition)
{
	//Msg("Start Loitering\n");
...
CBasePlayer *CNPC_SuppressorFlyGenerator::HasPhysicsAttacker(float dt)
{
...
//-----------------------------------------------------------------------------
float CNPC_SuppressorFlyGenerator::GetMaxEnginePower()
{
	if (m_bHackedByAlyx)
	{
		return 2.0f;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::UpdatePanels(void)
{
	if (m_flEngineStallTime > gpGlobals->curtime)
	{
...
// Input  : hostile - 
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::ShowHostile(bool hostile /*= true*/)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::StartBurst(const Vector &vecDirection)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::StopBurst(bool bInterruptSchedule /*= false*/)
{
	if (m_flBurstDuration < gpGlobals->curtime)
		return;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_SuppressorFlyGenerator::SetEyeState(int state)
{
...
unsigned int CNPC_SuppressorFlyGenerator::PhysicsSolidMaskForEntity(void) const
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_SuppressorFlyGenerator::CreateVPhysics(void)
{
	if (HasSpawnFlags(SF_MANHACK_CARRIED))
		return false;
...
//
//-----------------------------------------------------------------------------
AI_BEGIN_CUSTOM_NPC(npc_suppressorflygenerator, CNPC_SuppressorFlyGenerator)
...
DECLARE_ACTIVITY(ACT_FLYGENERATOR_UNPACK);
...
(
	...
	"	Tasks"
	"		TASK_PLAY_SEQUENCE			ACTIVITY:ACT_FLYGENERATOR_UNPACK"
	"		TASK_SET_ACTIVITY			ACTIVITY:ACT_FLY"
	"	"
	//	"	Interrupts"
);
