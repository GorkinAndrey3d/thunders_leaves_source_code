...
ConVar sk_weapon_ar2comb_alt_fire_radius("sk_weapon_ar2_alt_fire_radius", "10");
ConVar sk_weapon_ar2comb_alt_fire_duration("sk_weapon_ar2_alt_fire_duration", "2");
ConVar sk_weapon_ar2comb_alt_fire_mass("sk_weapon_ar2_alt_fire_mass", "150");

//=========================================================
//=========================================================

BEGIN_DATADESC(CWeaponAR2Combine)
...
IMPLEMENT_SERVERCLASS_ST(CWeaponAR2Combine, DT_WeaponAR2Combine)
END_SEND_TABLE()

LINK_ENTITY_TO_CLASS(weapon_ar2_comb, CWeaponAR2Combine);
PRECACHE_WEAPON_REGISTER(weapon_ar2_comb);

acttable_t	CWeaponAR2Combine::m_acttable[] =
{
...
IMPLEMENT_ACTTABLE(CWeaponAR2Combine);

CWeaponAR2Combine::CWeaponAR2Combine()
{
	m_fMinRange1 = 65;
	m_fMaxRange1 = 2048;
...
void CWeaponAR2Combine::Precache(void)
{
	PrecacheSound("Weapon_AR2.ReloadComb");
	BaseClass::Precache();
	PrecacheParticleSystem("weapon_muzzle_smoke");
	UTIL_PrecacheOther("prop_combine_ball");
	UTIL_PrecacheOther("env_entity_dissolver");
	PrecacheSound("Weapon_AR2.ReloadComb");
	PrecacheSound("weapons/ar2/ar2_reload_comb.wav");
}

//-----------------------------------------------------------------------------
// Purpose: Handle grenade detonate in-air (even when no ammo is left)
//-----------------------------------------------------------------------------
void CWeaponAR2Combine::ItemPostFrame(void)
{
...
// Output : Activity
//-----------------------------------------------------------------------------
Activity CWeaponAR2Combine::GetPrimaryAttackActivity(void)
{
	if (m_nShotsFired < 2)
...
//			nDamageType - 
//-----------------------------------------------------------------------------
void CWeaponAR2Combine::DoImpactEffect(trace_t &tr, int nDamageType)
{
	CEffectData data;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponAR2Combine::DelayedAttack(void)
{
	...
	CreateCombineBall(vecSrc,
		vecVelocity,
		sk_weapon_ar2comb_alt_fire_radius.GetFloat(),
		sk_weapon_ar2comb_alt_fire_mass.GetFloat(),
		sk_weapon_ar2comb_alt_fire_duration.GetFloat(),
		pOwner);
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponAR2Combine::SecondaryAttack(void)
{
	if (m_bShotDelayed)
		return;
...
//-----------------------------------------------------------------------------
bool CWeaponAR2Combine::CanHolster(void)
{
	if (m_bShotDelayed)
		return false;
...
// Purpose: Override if we're waiting to release a shot
//-----------------------------------------------------------------------------
bool CWeaponAR2Combine::Reload(void)
{
	if (m_bShotDelayed)
		return false;
	...
	if (pPlayer)
	{
		if (m_iClip1 < 1)
		{
			...
			if (fRet)
			{
				...
				if (pEjectProp)
				{
					Vector vecOrigin = pPlayer2->GetAbsOrigin() + vecForward * 32 + Vector(0, 0, 35);
					QAngle vecAngles(0, pPlayer2->GetAbsAngles().y - 10, 0);
					pEjectProp->SetAbsOrigin(vecOrigin);
					pEjectProp->SetAbsAngles(vecAngles);
					pEjectProp->KeyValue("model", "models/weapons/w_package.mdl");
					...
					pEjectProp->SUB_FadeOut();
				}
				PrecacheSound("Weapon_AR2.ReloadComb");
				EmitSound("Weapon_AR2.ReloadComb");
				EmitSound("weapons/ar2/ar2_reload_comb.wav");
			}
			return fRet;
		}
		else
		{
			...
			if (fRet)
			{
				...
				if (pEjectProp)
				{
					...
					pEjectProp->SetAbsOrigin(vecOrigin);
					pEjectProp->SetAbsAngles(vecAngles);
					pEjectProp->KeyValue("model", "models/weapons/w_ar2_clip.mdl");
					pEjectProp->KeyValue("solid", "1");
					pEjectProp->KeyValue("targetname", "EjectProp");
					pEjectProp->KeyValue("spawnflags", "260");
					pEjectProp->KeyValue("fademindist", "200");
...
// Input  : *pOperator - 
//-----------------------------------------------------------------------------
void CWeaponAR2Combine::FireNPCPrimaryAttack(CBaseCombatCharacter *pOperator, bool bUseWeaponAngles)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponAR2Combine::FireNPCSecondaryAttack(CBaseCombatCharacter *pOperator, bool bUseWeaponAngles)
{
	WeaponSound(WPN_DOUBLE);
	...
	float flDuration = RemapValClamped(flAmmoRatio, 0.0f, 1.0f, 0.5f, sk_weapon_ar2comb_alt_fire_duration.GetFloat());
	float flRadius = RemapValClamped(flAmmoRatio, 0.0f, 1.0f, 4.0f, sk_weapon_ar2comb_alt_fire_radius.GetFloat());
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponAR2Combine::Operator_ForceNPCFire(CBaseCombatCharacter *pOperator, bool bSecondary)
{
	if (bSecondary)
	{
...
//			*pOperator - 
//-----------------------------------------------------------------------------
void CWeaponAR2Combine::Operator_HandleAnimEvent(animevent_t *pEvent, CBaseCombatCharacter *pOperator)
{
	switch (pEvent->event)
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponAR2Combine::AddViewKick(void)
{
...
//-----------------------------------------------------------------------------
const WeaponProficiencyInfo_t *CWeaponAR2Combine::GetProficiencyValues()
{
