...
#include "grenade_frag_combine.h"
#include "Sprite.h"
...
ConVar sk_plr_dmg_fraggrenade_combine("sk_plr_dmg_fraggrenade", "0");
ConVar sk_npc_dmg_fraggrenade_combine("sk_npc_dmg_fraggrenade", "0");
ConVar sk_fraggrenade_radius_combine("sk_fraggrenade_radius", "0");

#define GRENADE_MODEL "models/Weapons/w_grenade_combine.mdl"

class CGrenadeFragCombine : public CBaseGrenade
{
	DECLARE_CLASS(CGrenadeFragCombine, CBaseGrenade);

#if !defined( CLIENT_DLL )
	DECLARE_DATADESC();
#endif

	~CGrenadeFragCombine(void);
...
LINK_ENTITY_TO_CLASS(npc_grenade_frag_combine, CGrenadeFragCombine);

BEGIN_DATADESC(CGrenadeFragCombine)
...
// Purpose: 
//-----------------------------------------------------------------------------
CGrenadeFragCombine::~CGrenadeFragCombine(void)
{
}

void CGrenadeFragCombine::Spawn(void)
{
	Precache();

	SetModel("models/Weapons/w_grenade_combine.mdl");

	if (GetOwnerEntity() && GetOwnerEntity()->IsPlayer())
	{
		m_flDamage = sk_plr_dmg_fraggrenade_combine.GetFloat();
		m_DmgRadius = sk_fraggrenade_radius_combine.GetFloat();
	}
	else
	{
		m_flDamage = sk_npc_dmg_fraggrenade_combine.GetFloat();
		m_DmgRadius = sk_fraggrenade_radius_combine.GetFloat();
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CGrenadeFragCombine::OnRestore(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CGrenadeFragCombine::CreateEffects(void)
{
...
bool CGrenadeFragCombine::CreateVPhysics()
{
...
void CGrenadeFragCombine::VPhysicsUpdate(IPhysicsObject *pPhysics)
{
...
void CGrenadeFragCombine::Precache(void)
{
	PrecacheModel("models/Weapons/w_grenade_combine.mdl");
...
void CGrenadeFragCombine::SetTimer(float detonateDelay, float warnDelay)
{
	m_flDetonateTime = gpGlobals->curtime + detonateDelay;
	m_flWarnAITime = gpGlobals->curtime + warnDelay;
	SetThink(&CGrenadeFragCombine::DelayThink);
	SetNextThink(gpGlobals->curtime);

	CreateEffects();
}

void CGrenadeFragCombine::OnPhysGunPickup(CBasePlayer *pPhysGunUser, PhysGunPickup_t reason)
{
...
void CGrenadeFragCombine::DelayThink()
{
...
void CGrenadeFragCombine::SetVelocity(const Vector &velocity, const AngularImpulse &angVelocity)
{
...
int CGrenadeFragCombine::OnTakeDamage(const CTakeDamageInfo &inputInfo)
{
...
bool CGrenadeFragCombine::HandleInteraction(int interactionType, void *data, CBaseCombatCharacter* sourceEnt)
{
...
void CGrenadeFragCombine::InputSetTimer(inputdata_t &inputdata)
{
...
CBaseGrenade *Fraggrenade_Create_combine(const Vector &position, const QAngle &angles, const Vector &velocity, const AngularImpulse &angVelocity, CBaseEntity *pOwner, float timer, bool combineSpawned)
{
	...
	CGrenadeFragCombine *pGrenade = (CGrenadeFragCombine *)CBaseEntity::Create("npc_grenade_frag", position, angles, pOwner);
	...
	pGrenade->SetModel("models/Weapons/w_grenade_combine.mdl");
	return pGrenade;
}

bool Fraggrenade_WasPunted_combine(const CBaseEntity *pEntity)
{
	const CGrenadeFragCombine *pFrag = dynamic_cast<const CGrenadeFragCombine *>(pEntity);
	if (pFrag)
	{
