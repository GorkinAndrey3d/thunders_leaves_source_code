...
public:
	...
	int		GetMinBurst() { return 2; } //2
	int		GetMaxBurst() { return 5; } //5
	...
	{ 
		//return ((0.7 / (m_nShotsFired + 4.1)) + 0.052);
		return ((4.9 / (-m_nShotsFired + 60.1)) - 0.02);
	}	// 13.3hz //0.075

	//return ((0.7/(m_nShotsFired + 3.7))+0.052);
...
void CWeaponSMG1::Precache( void )
{
	...
	PrecacheModel("models/weapons/w_package.mdl");
	BaseClass::Precache();
}
...
void CWeaponSMG1::SecondaryAttack( void )
{
	...
	if (pPlayer->GetAmmoCount(m_iSecondaryAmmoType) > 1) //разные анимации выстрела: с перезарядкой, если это не последняя граната - HEVcrab
	{
		SendWeaponAnim(ACT_VM_SECONDARYATTACK);

		// и надо выждать время анимации 2.2 сек (уточни, если необходимо, сколько длится у тебя анимация) до следующих атак обоих типов - HEVcrab
		m_flNextPrimaryAttack = gpGlobals->curtime + 2.2f;

		m_flNextSecondaryAttack = gpGlobals->curtime + 2.2f;
	}
	else // и без перезарядки, если последняя - HEVcrab
	{
		SendWeaponAnim(ACT_VM_SECONDARYATTACK_NO_RELOAD);

		// и можно через полсекунды стрелять основной атакой - HEVcrab
		m_flNextPrimaryAttack = gpGlobals->curtime + 0.5f;

		// а это пусть остаётся на случай, если после выстрела подобрали подствольную гранату: стрелять гранатой снова можно через секунду - HEVcrab
		m_flNextSecondaryAttack = gpGlobals->curtime + 1.0f;
	}

	CSoundEnt::InsertSound( SOUND_COMBAT, GetAbsOrigin(), 1000, 0.5, GetOwner(), SOUNDENT_CHANNEL_WEAPON ); //0.2
