...
#define	SF_ANTLION_CORN				( 1 << 18 )
#define	SF_ANTLION_WORKER			( 1 << 19 )// Use the "worker" model

typedef CAI_BlendingHost< CAI_BehaviorHost<CAI_BlendedNPC> > CAI_BaseAntlionBase;

class C_AntlionNugget : public CItem
{
public:
	DECLARE_CLASS(C_AntlionNugget, CItem);
	virtual void Spawn(void);
	virtual void Precache(void);
	virtual void VphysicsCollision(int index, gamevcollisionevent_t *pEvent);
	virtual void Event_Killed(const CTakeDamageInfo &info);
	virtual bool VphysicsInFlesh(void);
...
public:
	...
	bool		isCorn(void) const { return HasSpawnFlags(SF_ANTLION_CORN); }
...
class CGrubNugget_corn : public CItem
{
public:
	DECLARE_CLASS(CGrubNugget_corn, CItem);
...
extern bool IsAntlionCorn(CBaseEntity *pEntity);
