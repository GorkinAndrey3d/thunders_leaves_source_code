...
BEGIN_DATADESC( CNPC_MetroPolice )
	...
	DEFINE_INPUTFUNC( FIELD_VOID, "EnableShield", CreateShield),
	DEFINE_INPUTFUNC(FIELD_VOID, "ClearChild", UnlinkChild),
...
void CNPC_MetroPolice::UnlinkChild(inputdata_t &inputdata)
{
...
void CNPC_MetroPolice::CreateShield(inputdata_t & inputdata)
{
	Vector vecOrigin;
	Vector SupOrigin = GetAbsOrigin();
	CTLSuppressorShield_Attach *shield = (CTLSuppressorShield_Attach *)CreateEntityByName("npc_suppressor_shield_attach");
		shield->Precache();
		DispatchSpawn(shield);
		shield->SetAbsOrigin(SupOrigin);
		shield->SetParent(this);
		PhysDisableEntityCollisions(shield, this);
		PhysEntityCollisionsAreDisabled(this, shield);
...
LINK_ENTITY_TO_CLASS(npc_suppressor_shield_attach, CTLSuppressorShield_Attach);
BEGIN_DATADESC(CTLSuppressorShield_Attach)
...
void CTLSuppressorShield_Attach::Precache(void)
{
	BaseClass::Precache();
}

void CTLSuppressorShield_Attach::Hit(const CTakeDamageInfo &info)
{

}

void CTLSuppressorShield_Attach::Spawn(void)
{
...
int CTLSuppressorShield_Attach::OnTakeDamage(const CTakeDamageInfo & info)
{
	...
	//DispatchParticleEffect(SHIELD_IMPACT_PARTICLE, vecSplortPos, GetAbsAngles());
	//DispatchParticleEffect(SHIELD_IMPACT_PARTICLE, vecSplortPos2, GetAbsAngles());
	//DispatchParticleEffect(SHIELD_IMPACT_PARTICLE, vecSplortPos3, GetAbsAngles());
...
void CTLSuppressorShield_Attach::AbsoluteAngleThink(void)
{
...
void CTLSuppressorShield_Attach::InputToggle(inputdata_t &inputData)
{
	if (!m_bActive)
	{
		SetThink(&CTLSuppressorShield_Attach::AbsoluteAngleThink);
		SetNextThink(gpGlobals->curtime + 0.01f);
