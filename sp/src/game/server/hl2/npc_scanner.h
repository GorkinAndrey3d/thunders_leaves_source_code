...
class ScannerHealthSmoke : public  CBaseAnimating, public CDefaultPlayerPickupVPhysics
{
public:
	DECLARE_CLASS(ScannerHealthSmoke, CBaseAnimating);
	DECLARE_DATADESC();
...
public:
	...
	//Thunder`s Leaves
	void            ExplodeProcess(inputdata_t &inputdata);
	void            HealthSmokeProcess(inputdata_t &inputdata);
	void            PoisonSmokeProcess(inputdata_t &inputdata);
	float           m_flNextChangeTime;
	float           m_flNextSmokeTime;
	float           m_flAlertTime;
	float           m_flSuicide;
	int           m_flSuicide_num_01 = 0;
	float           m_flSuicide_num_02 = 0;  
	int             SmokeState = 0;
...
public:
    ...
    /*Thunder`s Leaves*/
	void            SuicideThink(void);
	void            SmokeThink(void);
	void            ForceAttentionSignal(void);
	void            ForceExplode(void);
	void            AssertSprite(void);
	CHandle<CSprite>	m_hAlertSprite;
	bool            AlertSpriteBool = false;
