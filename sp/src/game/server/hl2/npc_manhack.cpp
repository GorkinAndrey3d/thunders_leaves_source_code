...
bool CNPC_Manhack::OverrideMove( float flInterval )
{
	...
	{
		///arget(flInterval, m_vForceMoveTarget);
	}
	// -----------------------------------------------------------------
...
void CNPC_Manhack::MoveToTarget(float flInterval, const Vector &vMoveTarget)
	{
		...
		float flDist = VectorNormalize(targetDir) - 100;
...
void CNPC_Manhack::StartEye( void )
{
	...
	if ( m_pEyeGlow == NULL )
	{
		...
		{
			m_pEyeGlow->SetTransparency( kRenderTransAdd, 255, 190, 0, 95, kRenderFxNoDissipation );
			m_pEyeGlow->SetColor( 255, 190, 0 );
		}
...
void CNPC_Manhack::InputBreak(inputdata_t &inputdata)
{
	Break(inputdata.pActivator);
}

void CNPC_Manhack::Break(CBaseEntity *pBreaker)
{
	for (int i = 0; i < 3; i++)
	{
		Vector sparkPos = GetAbsOrigin();
