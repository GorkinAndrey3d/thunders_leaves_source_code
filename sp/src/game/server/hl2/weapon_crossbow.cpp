...
DEFINE_FIELD(m_flDelayedFire, FIELD_TIME), // for secondary attack: AR2-like warmup + fire a 3x faster bolt
DEFINE_FIELD(m_bShotDelayed, FIELD_BOOLEAN),
DEFINE_FIELD(m_bFastBolt, FIELD_BOOLEAN),
...
void CWeaponCrossbow::SecondaryAttack(void)
{
	...
	m_bShotDelayed = true;
	m_flNextPrimaryAttack = m_flNextSecondaryAttack = gpGlobals->curtime + 0.5f + SequenceDuration(ACT_VM_PRIMARYATTACK);
...
void CWeaponCrossbow::DelayedAttack(void)
{
	...
	SendWeaponAnim(ACT_VM_PRIMARYATTACK); // same firing animation as the primary attack, the AR2-like fidgeting is over
...
void CWeaponCrossbow::FireBolt(void)
{
	...
	{
		pBolt->SetAbsVelocity(vecAiming * BOLT_WATER_VELOCITY * velocity_multiplier);
	}
	else
	{
		pBolt->SetAbsVelocity(vecAiming * BOLT_AIR_VELOCITY * velocity_multiplier);
	}
