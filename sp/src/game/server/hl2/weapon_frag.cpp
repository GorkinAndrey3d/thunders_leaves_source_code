...
void CWeaponFrag::PrimaryAttack( void )
{
	...
	pPlayer->ViewPunch(QAngle(-0.5, random->RandomFloat(2, 3), 0));
	Msg("WoOOooW 2");
...
void CWeaponFrag::ItemPostFrame( void )
{
	...
	if (pOwner->m_nButtons & IN_RELOAD)
	{
		...
		pPlayer->ViewPunch(QAngle(-0.5, random->RandomFloat(0.5, 1), 0));
		SendWeaponAnim(ACT_VM_READY);
	}
		...
		if (pOwner)
		{
			...
			case GRENADE_PAUSED_PRIMARY:
				...
				else
				{
					...
					pPlayer->ViewPunch(QAngle(ThrowAngle, random->RandomFloat(0.01, 0.02), 0));
					Msg("WoOOooW 1");
					ThrowAngle -= 0.005;
				}
				break;
