...
ConVar	sk_combine_s_health_heavy("sk_combine_s_health_heavy", "0");
ConVar	sk_combine_s_heavy_kick("sk_combine_s_heavy_kick", "0");
...
void CNPC_CombineS::Spawn( void )
{
	...
	if (IsHeavy())
	{
		SetHealth(sk_combine_s_health_heavy.GetFloat());
		SetMaxHealth(sk_combine_s_health.GetFloat());
		SetKickDamage(sk_combine_s_heavy_kick.GetFloat());
	}
	else 
	{
...
void CNPC_CombineS::Precache()
{
	...
	if (!Q_stricmp(pModelName, "models/combine_soldier_heavy.mdl"))
	{
		m_fIsHeavy = true;
	}
	else
