...
#define	ANTLION_CORN_MODEL			"models/antlion_corn/antlion.mdl"
...
BEGIN_DATADESC(CGrubNugget_corn)
DEFINE_FIELD(m_nDenomination, FIELD_INTEGER),
END_DATADESC()

LINK_ENTITY_TO_CLASS(item_grubnugget_corn, CGrubNugget_corn);
//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
...
void CNPC_Antlion::Precache( void )
{
	...
	if ( isCorn() )
	{
		PrecacheModel(ANTLION_CORN_MODEL);
		PropBreakablePrecacheAll(MAKE_STRING(ANTLION_CORN_MODEL));
		PrecacheParticleSystem("blood_impact_antlion_01");
		PrecacheParticleSystem("AntlionGib");
		PrecacheParticleSystem("blood_impact_antlion_worker_01");
		PrecacheParticleSystem("antlion_gib_02");
		PrecacheParticleSystem("blood_impact_yellow_01");
		PrecacheParticleSystem("antlion_gib_02_gas");
		...
		PrecacheScriptSound("NPC_Antlion.JumpTouch");
	}
	else
	...
	PrecacheSound("npc/antlion/antlion_burst2.wav");
#endif
...
int CNPC_Antlion::GetNuggetDenomination(void)
{
...
void CNPC_Antlion::CreateNugget(void)
{
	CGrubNugget_corn *pNugget = (CGrubNugget_corn *)CreateEntityByName("item_grubnugget_corn");
	if (pNugget == NULL)
		return;
	...
	pNugget->SetModelScale(1.1);
	pNugget->GetModelScale();

	
	m_hCornSprite = CSprite::SpriteCreate("sprites/grubflare1.vmt", GetLocalOrigin(), false);

	Assert(m_hCornSprite);
	m_hCornSprite->TurnOn();
	m_hCornSprite->SetAbsOrigin(vecOrigin);
	m_hCornSprite->SetTransparency(kRenderWorldGlow, 156, 169, 121, 255, kRenderFxNoDissipation);
	m_hCornSprite->SetScale(1.0f);
	m_hCornSprite->SetGlowProxySize(16.0f);
	int nAttachment = LookupAttachment("glow");
	m_hCornSprite->SetParent(pNugget, nAttachment);
	m_hCornSprite->SetLocalOrigin(vec3_origin);
}
...
bool CNPC_Antlion::IsJumpLegal( const Vector &startPos, const Vector &apex, const Vector &endPos ) const
{
	...
	const float MIN_JUMP_CORN_DISTANCE = 5;
	...
	if (isCorn())
	{
		if ((endPos - GetAbsOrigin()).Length() < MIN_JUMP_CORN_DISTANCE)
			return false;
	}
	else
	{
			...
			if ( pBlocker && pBlocker->Classify() == CLASS_ANTLION )
			{
				...
				if ( pAntlion )
				{
					...
					//	NDebugOverlay::Box( pAntlion->GetAbsOrigin(), GetHullMins(), GetHullMaxs(), 0, 0, 255, 0, 2 );
						...
						if (isCorn())
						{
							pAntlion->m_flNextJumpPushTime = gpGlobals->curtime + 0.01f;
						}
						else 
							pAntlion->m_flNextJumpPushTime = gpGlobals->curtime + 2.0f;
					}
...
void CNPC_Antlion::DoPoisonBurst()
{
	if ( GetWaterLevel() < 2 )
	{
		CTakeDamageInfo info( this, this, sk_antlion_worker_burst_damage.GetFloat(), DMG_BLAST_SURFACE | ( ANTLION_WORKER_BURST_IS_POISONOUS() ? DMG_CRUSH : DMG_CRUSH) );

		RadiusDamage( info, GetAbsOrigin(), 300.0, CLASS_NONE, this );

		DispatchParticleEffect( "antlion_gib_02", WorldSpaceCenter(), GetAbsAngles() );
		DispatchParticleEffect("antlion_gib_02_gas", WorldSpaceCenter(), GetAbsAngles());
	}
	else
	{
		CEffectData	data;
...
bool IsAntlionCorn(CBaseEntity *pEntity)
	{
		// Must at least be valid and an antlion
		return (pEntity != NULL &&
			pEntity->Classify() == CLASS_ANTLION &&
			pEntity->HasSpawnFlags(SF_ANTLION_CORN) &&
			dynamic_cast<CNPC_Antlion *>(pEntity) != NULL);	// Save this as the last step
	}
...
void CNPC_Antlion::CreateGlow(void)
{
...
void C_AntlionNugget::Spawn(void)
{
	Precache();
...
void C_AntlionNugget::Precache(void)
{
...
void C_AntlionNugget::VphysicsCollision(int index, gamevcollisionevent_t * pEvent)
{
	int damageType;
...
void C_AntlionNugget::Event_Killed(const CTakeDamageInfo & info)
{
	AddEffects(EF_NODRAW);
...
bool C_AntlionNugget::VphysicsInFlesh(void)
{
	return false;
}

bool C_AntlionNugget::MyTouch(CBasePlayer * pPlayer)
{
	int nHealthToGive;
	nHealthToGive = 10;
...
void CGrubNugget_corn::Spawn(void)
{
	Precache();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CGrubNugget_corn::Precache(void)
{
	PrecacheModel("models/grub_nugget_small.mdl");
	PrecacheModel("models/grub_nugget_medium.mdl");
	PrecacheModel("models/grub_nugget_large.mdl");
...
//-----------------------------------------------------------------------------
bool CGrubNugget_corn::VPhysicsIsFlesh(void)
{
	return false;
}
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CGrubNugget_corn::MyTouch(CBasePlayer *pPlayer)
{
...
//			*pEvent - 
//-----------------------------------------------------------------------------
void CGrubNugget_corn::VPhysicsCollision(int index, gamevcollisionevent_t *pEvent)
{
	int damageType;
	float damage = CalculateDefaultPhysicsDamage(index, pEvent, 1.0f, true, damageType);
	if (damage > 5.0f)
...
// Input  : &info - 
//-----------------------------------------------------------------------------
void CGrubNugget_corn::Event_Killed(const CTakeDamageInfo &info)
{
	AddEffects(EF_NODRAW);
	DispatchParticleEffect("antlion_spit_player", GetAbsOrigin(), QAngle(-90, 0, 0));
	EmitSound("NPC_Antlion_Grub.Explode");
