...
/* HEALTH SMOKE HEALTH SMOKE HEALTH SMOKE HEALTH SMOKE HEALTH SMOKE HEALTH SMOKE */
/* HEALTH SMOKE HEALTH SMOKE HEALTH SMOKE HEALTH SMOKE HEALTH SMOKE HEALTH SMOKE */
/* HEALTH SMOKE HEALTH SMOKE HEALTH SMOKE HEALTH SMOKE HEALTH SMOKE HEALTH SMOKE */
/* HEALTH SMOKE HEALTH SMOKE HEALTH SMOKE HEALTH SMOKE HEALTH SMOKE HEALTH SMOKE */


LINK_ENTITY_TO_CLASS(npc_ScannerHealthSmoke, ScannerHealthSmoke);

// Start of our data description for the class
BEGIN_DATADESC(ScannerHealthSmoke)
...
//-----------------------------------------------------------------------------
void ScannerHealthSmoke::Precache(void)
{
	PrecacheModel(ENTITY_MODEL);
	PrecacheParticleSystem("tl_smoke_healthh");
	PrecacheParticleSystem("tl_smoke_poison");
	BaseClass::Precache();
}
...
//-----------------------------------------------------------------------------
void ScannerHealthSmoke::Spawn(void)
{
	Precache();
	SetSolid(SOLID_NONE);
	SetBlocksLOS(false);
	...
	SetTouch(&ScannerHealthSmoke::BrushTouch);
...
void ScannerHealthSmoke::SetSmokeType(int type)
{
	SmokeType = type;
}

unsigned int ScannerHealthSmoke::PhysicsSolidMaskForEntity(void) const
{
...
//-----------------------------------------------------------------------------
void ScannerHealthSmoke::MoveThink(void)
{
	...
	if (m_flNextChangeTime < gpGlobals->curtime)
	{
		...
		if (SmokeType == 2)
		{
			DispatchParticleEffect("tl_smoke_poison", GetAbsOrigin(), GetAbsAngles());
			//m_flNextChangeTime = gpGlobals->curtime + random->RandomFloat(0.1f, 0.1f);
			//return;
		}
...
void ScannerHealthSmoke::Toggle()
{
	if (!m_bActive)
	{
		m_flFadeTime = gpGlobals->curtime + float(2.0f);
		// Start thinking
		SetThink(&ScannerHealthSmoke::MoveThink);
		SetNextThink(gpGlobals->curtime + 0.05f);
...
void ScannerHealthSmoke::BrushTouch(CBaseEntity *pOther)
{
	UTIL_Remove(this);
}
...
#define SPRITE_ALERT "sprites/glow1.vmt"
...
//	DEFINE_FIELD( m_hSpotlightTarget,		FIELD_EHANDLE ),
	...
	/* Thunder`s Leaves */
	DEFINE_INPUTFUNC(FIELD_VOID, "ExplodeProcess", ExplodeProcess),
	DEFINE_INPUTFUNC(FIELD_VOID, "HealthSmokeProcess", HealthSmokeProcess),
	DEFINE_INPUTFUNC(FIELD_VOID, "PoisonSmokeProcess", PoisonSmokeProcess),
...
void CNPC_CScanner::ExplodeProcess(inputdata_t &inputdata)
{
	m_flSuicide = 1;
...
void CNPC_CScanner::HealthSmokeProcess(inputdata_t & inputdata)
{
	SmokeState = 1;
}

void CNPC_CScanner::PoisonSmokeProcess(inputdata_t & inputdata)
{
	SmokeState = 2;
}
...
void CNPC_CScanner::NPCThink(void)
{
	...
	{
		if (m_bIsClawScanner == true && m_ClawCheck == false)
		{
			m_iHealth = 25;
			m_ClawCheck = true;
		}
...
void CNPC_CScanner::Precache(void)
{
	...
	if( m_bIsClawScanner )
	{
		...
		PrecacheMaterial("sprites/glow1.vmt");
		PrecacheScriptSound("NPC_SScanner.ExplodeAlert");
	...
	else
	{
		...
		PrecacheScriptSound("NPC_CScanner.ExplodeAlert");
...
void CNPC_CScanner::SuicideThink(void)
{
		if (m_flSuicide == 1 && m_flNextChangeTime < gpGlobals->curtime && m_flSuicide_num_01 < 4)
		{
			AssertSprite();
			//-----------SPRITE
			m_hAlertSprite->TurnOn();
			//-------------

			ForceAttentionSignal();
			m_flNextChangeTime = gpGlobals->curtime + float(0.8f);
			m_flAlertTime = gpGlobals->curtime + float(0.2f);
			m_flSuicide_num_01++;
			AlertSpriteBool = true;
			if (m_flSuicide_num_01 == 4)
			{
				m_flSuicide_num_02++;
			}

		}
		if (m_flSuicide == 1 && m_flNextChangeTime < gpGlobals->curtime && m_flSuicide_num_02 > 0 && m_flSuicide_num_02 < 8)
		{
			m_hAlertSprite->TurnOn();
			ForceAttentionSignal();
			m_flNextChangeTime = gpGlobals->curtime + float(0.2f);
			m_flAlertTime = gpGlobals->curtime + float(0.05f);
			AlertSpriteBool = true;
			m_flSuicide_num_02++;
			if (m_flSuicide_num_02 == 8)
			{
				ForceExplode();
				Gib();
				m_iHealth = 0;
		...
		if (m_flAlertTime < gpGlobals->curtime && AlertSpriteBool == true)
		{
			AlertSpriteBool = false;
			m_hAlertSprite->TurnOff();
		}
		else
		{
			return;
...
void CNPC_CScanner::SmokeThink(void)
{
	if (SmokeState == 1 && m_flNextSmokeTime < gpGlobals->curtime)
	{
		ScannerHealthSmoke *pSmoke = (ScannerHealthSmoke *)CreateEntityByName("npc_ScannerHealthSmoke");
		...
		pSmoke->SetSmokeType(1);
		pSmoke->SetAbsOrigin(vecOrigin);
		DispatchSpawn(pSmoke);
		

		m_flNextSmokeTime = gpGlobals->curtime + float(0.3f);
	}
	if (SmokeState == 2 && m_flNextSmokeTime < gpGlobals->curtime)
	{
		ScannerHealthSmoke *pSmoke = (ScannerHealthSmoke *)CreateEntityByName("npc_ScannerHealthSmoke");
...
void CNPC_CScanner::ForceAttentionSignal(void)
{
	ScannerEmitSound("ExplodeAlert");
}

void CNPC_CScanner::ForceExplode(void)
{
	ExplosionCreate(WorldSpaceCenter(), GetAbsAngles(), this, 200, 200, true, 200, false, false);
}

void CNPC_CScanner::AssertSprite(void)
{
	m_hAlertSprite = CSprite::SpriteCreate(SPRITE_ALERT, GetLocalOrigin(), false);
	Assert(m_m_hAlertSprite);
	if (m_hAlertSprite == NULL)
	{
		return;
	}
	m_hAlertSprite->SetTransparency(kRenderWorldGlow, 255, 40, 40, 255, kRenderFxNoDissipation);
	m_hAlertSprite->SetScale(1.0f);
	m_hAlertSprite->SetGlowProxySize(16.0f);
	int nAttachment = LookupAttachment("glow");
	m_hAlertSprite->SetParent(this, nAttachment);
	m_hAlertSprite->SetLocalOrigin(vec3_origin);
	m_hAlertSprite->SetThink(NULL);
	m_hAlertSprite->SetNextThink(TICK_NEVER_THINK);
	m_hAlertSprite->TurnOff();
}
