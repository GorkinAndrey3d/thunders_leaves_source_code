...
DEFINE_FIELD(m_fIsHeavy, FIELD_BOOLEAN),
	...
	case TASK_COMBINE_GET_PATH_TO_FORCED_GREN_LOS:
		{
			...
			if ( GetTaskInterrupt() > 0 )
			{
				...
				if (IsHeavy())
				{
					AI_NavGoal_t goal(m_vInterruptSavePosition, ACT_WALK, AIN_HULL_TOLERANCE);
...
void CNPC_Combine::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if( GetActivity() == ACT_RANGE_ATTACK2 )
	{
		if( m_iLastAnimEventHandled != COMBINE_AE_GREN_TOSS )
		{
			...
			CBaseEntity *pItem = DropItem( "weapon_frag_combine", vecStart, RandomAngle(0,360) );
		...
		case COMBINE_AE_GREN_TOSS:
			{
				...
				if( m_NPCState == NPC_STATE_SCRIPT )
				{
					...
					Fraggrenade_Create_combine( vecStart, vec3_angle, vecThrow, vecSpin, this, COMBINE_GRENADE_TIMER, true );
				}
				else
				{
					// Use the Velocity that AI gave us.
					Fraggrenade_Create_combine( vecStart, vec3_angle, m_vecTossVelocity, vecSpin, this, COMBINE_GRENADE_TIMER, true );
					m_iNumGrenades--;
				}
		...
		case COMBINE_AE_GREN_DROP:
			{
				...
				Fraggrenade_Create_combine( vecStart, vec3_angle, m_vecTossVelocity, vec3_origin, this, COMBINE_GRENADE_TIMER, true );
				m_iNumGrenades--;
			}
			handledEvent = true;
			break;
